...
/**
 * Hands the Source-to-glTF conversion back to the scene's root node, undoing what
 * ValveResourceFormat bakes into an exported model.
 *
 * Shared by the two finishing passes: item-generator-glb.ts (every model) and
 * item-generator-agent-glb.ts (agents, which also need it applied before their inventory pose is
 * written into the joints, so the pose's inch-valued translations land in the space they were
 * authored in).
 */
 ...
 * over an output tree.
 */
export function unbakeSourceConversion(document: Document): boolean {
