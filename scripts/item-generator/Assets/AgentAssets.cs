...
public static partial class AssetProcessor
{
    ...
    private static readonly string[] PatchArtworkProperties =
    [
        "g_tPatch0",
        "g_tPatch1",
        "g_tPatch2",
    ...
    private sealed record GlbMaterial(
        ...
        Dictionary<string, double> FloatParams,
        Dictionary<string, double[]> VectorParams,
        Dictionary<string, long> IntParams,
        Dictionary<string, string> GltfSlots
    );
    ...
    {
        if (!ctx.AgentModelExports.TryGetValue(vpkPath, out var export))
            return;
        ...
        var tiers = Config.IsTextureOptimizationSkipped()
            ? []
            : CharacterTextureOptimization.ResolveTextureTiers(
                materials.Select(m =>
                    (
                        m.Path,
                        m.ShaderName,
                        (IReadOnlyDictionary<string, string>)m.TextureParams,
        ...
        var rebuiltOrms = await RebuildAnisotropicRoughness(
            ctx,
            glbPath,
            model,
            materials,
        ...
        var sources = images
            ...
            .Select(image =>
                (
                    image.Name,
                    PngPath: rebuiltOrms.GetValueOrDefault(image.Name)
                        ?? Path.Combine(glbDir, Uri.UnescapeDataString(image.Uri))
                )
            )
            .Concat(
                await DecompileLooseTextures(
                    ctx,
                    glbPath,
                    model,
                    images,
    ...
    private static List<string> ReadDrawnPatchBackings(PendingModelTask model)
    {
        ...
        if (!document.RootElement.TryGetProperty("patchSlots", out var slots))
            return [];
        var drawn = new List<string>();
        foreach (var slot in slots.EnumerateArray())
            if (
                slot.TryGetProperty("backingScale", out var scale)
                && scale.GetDouble() != 0
                && slot.TryGetProperty("backing", out var backing)
                && backing.GetString() is { Length: > 0 } name
            )
                drawn.Add(name);
        return drawn;
    }
    ...
    private static IEnumerable<string> ReadEyeTextures(List<GlbMaterial> materials) =>
        materials
            ...
            .SelectMany(material =>
                EyeTextureProperties.Select(property =>
    ...
    private static async Task<List<(string Name, string PngPath)>> DecompileLooseTextures(
        ...
        string glbPath,
        PendingModelTask model,
        List<(string Name, string Uri)> images,
        ...
        [
            .. textures.Select(texture =>
            {
                var pngPath = Path.Combine(
                    ...
                    texture.VpkPath[..^"vtex_c".Length] + "png"
                );
    ...
    )
    {
        ...
        foreach (var material in materials)
        {
            if (!material.GltfSlots.TryGetValue("metallicRoughnessTexture", out var orm))
                continue;
            var source = "";
            if (material.IntParams.GetValueOrDefault("F_ANISOTROPIC_GLOSS") != 0)
                source =
                    material.TextureParams.GetValueOrDefault("g_tAnisoGloss")
                    ?? throw new InvalidOperationException(
        ...
        foreach (var (orm, gloss) in rebuilds)
        {
            ...
            var ormPath =
                image.Uri == null ? null : Path.Combine(glbDir, Uri.UnescapeDataString(image.Uri));
            if (ormPath == null || !File.Exists(ormPath))
                continue;
    ...
    private static void ResolveAgentPatchSlots(ItemGeneratorContext ctx)
    {
        ...
        foreach (var (vpkPath, model) in agents)
        {
            ...
            using var document = JsonDocument.Parse(File.ReadAllText(modelDataPath));
            if (ConvertJsonElement(document.RootElement) is not Dictionary<string, object?> data)
                continue;
    ...
    private static List<object?>? BuildPatchSlots(
        Dictionary<string, object?> modelData,
        List<GlbMaterial> materials,
        string modelName
    )
    {
        var hosts = materials
            .Where(m =>
                string.Equals(
                    m.ShaderName,
                    CharacterTextureOptimization.CharacterShader,
        ...
        foreach (var path in order)
        {
            var host = hosts.FirstOrDefault(m =>
                string.Equals(
                    MaterialPaths.NormalizeMaterialResourcePath(m.Path),
                    MaterialPaths.NormalizeMaterialResourcePath(path),
                    StringComparison.OrdinalIgnoreCase
                )
            );
            if (host != null && !ordered.Contains(host))
                ordered.Add(host);
        }
            ...
            for (var local = 0; local < CharacterPatchSlots; local++)
            {
                ...
                slots.Add(
                    new Dictionary<string, object?>
                    {
                        ...
                        ["offset"] = Vector2(
                            material.VectorParams.GetValueOrDefault($"g_vPatch{local}Offset")
                        ),
                        ["scale"] = material.FloatParams.GetValueOrDefault(
                            $"g_flPatch{local}Scale",
                            0
                        ),
                        ["rotation"] = material.FloatParams.GetValueOrDefault(
                            $"g_flPatch{local}Rotation",
                            0
                        ),
                        ["squash"] = material.FloatParams.GetValueOrDefault(
                            $"g_flPatch{local}Squash",
                            1
                        ),
                        ["backingScale"] = material.FloatParams.GetValueOrDefault(
                            $"g_flPatch{local}BackingScale",
                            0
                        ),
                        ["backing"] = material.TextureParams.GetValueOrDefault(
                            $"g_tPatch{local}Backing"
                        ),
                        ...
                        ["enabled"] =
                            material.IntParams.GetValueOrDefault($"g_bEnablePatch{local}", 0) != 0,
                    }
                );
            }
        }
    ...
    private static Dictionary<string, object?> ModelKeyValues(
        Dictionary<string, object?> modelData
    ) =>
    ...
    {
        if (keyValues.GetValueOrDefault(key) is not Dictionary<string, object?> block)
            return [];
        return
        [
            .. block
    ...
    private static List<GlbMaterial> ReadGlbMaterials(string glbPath)
    {
        ...
        if (!doc.RootElement.TryGetProperty("materials", out var array))
            return materials;
    ...
    private static List<string?> ReadTextureImageNames(JsonElement root)
    {
        if (!root.TryGetProperty("textures", out var textures))
            return [];
        var images = root.TryGetProperty("images", out var i)
            ? [.. i.EnumerateArray()]
    ...
    /// </summary>
    private static Dictionary<string, string> ReadGltfSlots(
    ...
    )
    {
        ...
        {
            if (
                owner.TryGetProperty(slot, out var info)
                && info.TryGetProperty("index", out var index)
                && index.GetInt32() < imageNames.Count
                && imageNames[index.GetInt32()] is { Length: > 0 } image
            )
                slots[slot] = image;
        }
    ...
    private static Dictionary<string, string> ReadTextureResources(string glbPath)
    {
        ...
        foreach (var material in array.EnumerateArray())
        {
            if (
                !material.TryGetProperty("extras", out var extras)
                || !extras.TryGetProperty("vmat", out var vmat)
                || !vmat.TryGetProperty("TextureParams", out var textures)
            )
                continue;
